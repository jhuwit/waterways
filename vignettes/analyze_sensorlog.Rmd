...
One of the main issues with the daily level if you typically want to cross-reference GPS with activity, so we will keep the data at a minute level for now.
...
### Getting Times
You can easily manipulate the time variables to get the date, hour, and minute level data, since you many times want to plot the data by hour/minute and facet by day.  Here is an example function `create_date_hour_minute`:  
...
The `actibase::acti_separate_times` function performs these if the `actibase` package is available.
