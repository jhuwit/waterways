...
To also summarize the activity profile for different levels of proximity to a fixed point we will read in the GT3X data. This data was captured using an ActiGraph (Pensacola FL) GT9X device. The data is stored in the `extdata` folder of the package, similar to the SensorLog data. We get the path via `ww_example_gt3x_file`.  
...
file_gt3x
ag = ww_read_gt3x(file_gt3x, verbose = FALSE)
head(ag)
```
